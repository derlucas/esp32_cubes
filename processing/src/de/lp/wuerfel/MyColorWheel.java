...
public class MyColorWheel extends Controller< MyColorWheel > {
    ...
    int _sideHandleHeight = 20;
    private double[] hsl = new double[] { 1.0 , 1.0 , 1.0 };


    public MyColorWheel( ControlP5 theControlP5 , String theName ) {
        this( theControlP5 , theControlP5.getDefaultTab( ) , theName , 0 , 0 , autoWidth , autoHeight );
        theControlP5.register( theControlP5.papplet , theName , this );
    }

    public MyColorWheel( ControlP5 theControlP5 , ControllerGroup< ? > theParent , String theName , int theX , int theY , int theWidth , int theHeight ) {
        super( theControlP5 , theParent , theName , theX , theY , theWidth , theHeight );
        theControlP5.register(theControlP5.papplet, theName, this);

        _myColorResources = new HashMap<>( );
    ...
    public MyColorWheel scrolled( int theRotationValue ) {
        if ( isVisible ) {
            double x = getPointer( ).x( );
            double d1 = ( ( getWidth( ) / 2 ) * scalar ) + 1;
            double w = ( getWidth( ) - ( d1 * 2 ) ) / 2;
    ...
    public MyColorWheel setHSL( double theH , double theS , double theL ) {
        setHue( theH );
        setSaturation( theS );
        setLightness( theL );
        return this;
    ...
    public MyColorWheel setRGB( int theColor ) {
    ...
    @Override
    public MyColorWheel updateDisplayMode( int theMode ) {
        return updateViewMode( theMode );
    }

    public MyColorWheel updateViewMode( int theMode ) {
        _myDisplayMode = theMode;
        switch ( theMode ) {
            case ( DEFAULT ):
            case ( IMAGE ):
            case ( CUSTOM ):
                _myControllerView = new MyColorWheel.ColorWheelView( );
            default:
                break;
    ...
    public int b( ) {
        return _myColorValue & 0x000000ff;
    }

    private class ColorWheelView implements ControllerView< MyColorWheel > {

        public void display( PGraphics theGraphics , MyColorWheel theController ) {
            ...
            if ( isInfo ) {
                _myInfoLabel.setText( String.format( "RGB  %d  %d  %d\nHSL %d  %.2f  %.2f " , r( ) , g( ) , b( ) , ( int ) ( hsl[ 0 ] * 360 ) , hsl[ 1 ] , hsl[ 2 ] ) );
                _myInfoLabel.draw( theGraphics , 25 , 0 , theController );
            }
            theGraphics.fill( 20 );
            theGraphics.pushMatrix( );
            theGraphics.rect(0,0, _sideHandleHeight, getHeight() - ( yoff * 2 ));
            theGraphics.popMatrix( );
